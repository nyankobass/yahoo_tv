...
class API:
    ...
    def silent(self, enable: bool = True):
        '''ログ出力の設定
        :param bool enable: Trueならログ出力を抑制する
        '''
        if enable:
    ...
    def __verify_dt(dt: datetime.datetime):
        ...
        if yesterday > dt:
            LOGGER.error("過去の番組表は取得できません。")
            return False

        elif yesterday + datetime.timedelta(weeks=1) < dt:
            LOGGER.error("一週間以上先の番組表は取得できません。")
            return False
